...
MainMenuState::MainMenuState()
{
	...
	// zh-TW 簽名圖 (原來是個胖仔 / wicanr2)
	// 48x24 paletted PNG，放右下角 BACK01.SCR 背景上不擋按鈕
	// 設計：chibi 圓臉戴綠眼鏡 + "WICANR2" + 「胖仔」
	// 用 GEOSCAPE palette block 14 (橄欖→暗棕) + idx 32 (亮綠眼鏡點綴)
	_sigBadge = new Surface(48, 24, 270, 174);
	...
	add(_sigBadge);  // raw surface (uses parent state's palette)

	// Load signature image from common/Resources/
	try
	{
	...
	{
		// missing PNG is non-fatal; signature just won't render
	}
