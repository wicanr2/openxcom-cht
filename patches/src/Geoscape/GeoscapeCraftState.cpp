...
GeoscapeCraftState::GeoscapeCraftState(Craft *craft, Globe *globe, Waypoint *waypoint) : _craft(craft), _globe(globe), _waypoint(waypoint)
{
	...
	// CHT v2.22 T3: row h 9->11, pitch 8->10. _txtBase y=52 kept; subsequent rows
	// y = 62/72/82/92/102/112 (last row底邊 123, _btnBase y=124 → 1 px gap).
	// _txtRedirect (h=17, big, conditional overlay) kept at y=108 — its overlap
	// with W1/W2 rows matches original behavior when _waypoint != 0.
	_txtBase = new Text(210, 11, 32, 52);
	_txtSpeed = new Text(210, 11, 32, 62);
	_txtMaxSpeed = new Text(210, 11, 32, 72);
	_txtAltitude = new Text(210, 11, 32, 82);
	_txtFuel = new Text(130, 11, 32, 92);
	_txtDamage = new Text(80, 11, 164, 92);
	_txtW1Name = new Text(130, 11, 32, 102);
	_txtW1Ammo = new Text(80, 11, 164, 102);
	_txtW2Name = new Text(130, 11, 32, 112);
	_txtW2Ammo = new Text(80, 11, 164, 112);
	_txtRedirect = new Text(230, 17, 13, 108);
	_txtSoldier = new Text(80, 11, 164, 72);
	_txtHWP = new Text(80, 11, 164, 82);
