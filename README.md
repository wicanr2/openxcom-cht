...
還記得嗎？1990 年代的某個深夜，房間裡只剩 CRT 螢幕的微藍光，14 吋螢幕跳出滿屏的「`UFOPAEDIA`」「`ALIEN SUB PEN`」「`MAGNETIC NAVIGATION`」⋯⋯ 你一手握著《電腦玩家》別冊翻譯，一手指揮一群叫不出名字的水兵潛入海底，那時候沒有 wiki、沒有 Discord、沒有 GameFAQ，只有書局架上薄薄一本第三波出版的繁中手冊，跟 BBS 留言板上前輩留下的零碎攻略。

三十年過去了。

那群指揮官早就變成了 IT 工程師、變成了爸爸媽媽、變成了「下班還有力氣再開一場戰役」的中年玩家。但每次重灌 Steam、看到那滿屏英文，心裡還是會問一句：**這款神作什麼時候才會完整中文化？**

這個專案就是答案。我修好了 OpenXcom 字型太小、Geoscape 時鐘「星期五」被切、UFOpaedia 文字色跟天空融成一片的老問題（**17 個 source patch、12 px WQY 點陣字、PaletteShift 公式調整都記錄在 [`docs/`](docs/) 裡**）。我翻出了塵封三十年的 [1994](docs/) / [1995](docs/) 第三波官方手冊，把當年阮建成、維京工作室翻譯的「**電漿**」「**水族人**」「**潛艇修理塢**」「**黃貂魚式飛彈**」這些譯名重新校對、復原、修補誤譯。連 **34 國士兵的英文名**都用 1990 年代聯合報國際版的音譯規範，一個個拼成中文 — 「`Wang Wei`」變回「**王偉**」、「`Fernando Guedes`」變成「**費南多·古艾迪斯**」、「`Yılmaz`」變成「**伊爾馬茲**」。

整份 README、整個排版、整套技術文件，我都用 [Claude Code](https://claude.com/claude-code) 重新編輯了一輪 —— 希望這份漢化檔不只是「能玩」，而是一份 **30 年後給每一位老幽浮迷的回信**：那些深夜熬到三點才在賽多尼亞炸掉外星大腦的青春、那些隔天上學還在走廊跟同學說「昨天我練到上尉了」的記憶，現在你可以用母語再玩一次了。
