...
武器表這節有 30 年來最重要的一條警告：**SONIC 譯「音速」是物理誤譯**。維京 1995 那年把 sonic（聲學的）混淆成 supersonic（音速以上），這在民國 84 年的台灣中學物理課本還寫「音速」當作 sonic 翻譯時並不算離譜——譯者跟著時代用詞。30 年後本專案修正為「**聲波**」是物理學正確用法（sonic gun=聲波槍，發出機械振動波；不是 supersonic gun=音速以上的槍）。**這也是 5 條「不要回退」警告之一**。另一條值得注意的是 **THERMAL TAZER「冷凍電擊棒」** ——這個譯名 v2.20 已修正為「**熱能電擊棒**」（THERMAL=熱、TAZER=電擊，破解水中低溫防禦設計），早期社群誤譯把方向反了。
