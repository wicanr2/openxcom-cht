#!/bin/bash
# v2.21 dual AppImage builder — UFO + TFTD variants
# (same as v2.20 but output names → v2.21)
set -e
...
build_variant() {
  ...
  echo "==> Building v2.21-$VARIANT (mod=$MOD_ID)"

  local AD=/tmp/appimg/OpenXcom-CHT-v2.21-$VARIANT.AppDir
  rm -rf $AD
...
PY_EOF
  ...
  local OUT_NAME="OpenXcom-CHT-v2.21-$VARIANT-x86_64.AppImage"
...
echo ""
echo "=== Final v2.21 artifacts ==="
ls -lh $OUT/OpenXcom-CHT-v2.21-*
