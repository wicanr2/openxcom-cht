#!/bin/bash
# v2.20 dual AppImage builder — UFO + TFTD variants
set -e
...
test -f $EXE || { echo "missing Linux EXE — run cmake build first"; exit 1; }
...
test -f $APPIMG_TOOL || { echo "appimagetool download failed"; exit 1; }

build_variant() {
  local VARIANT=$1     # UFO / TFTD
  local MOD_ID=$2      # xcom1 / xcom2
  local DATA_DIR=$3    # UFO / TFTD

  echo "=========================================="
  echo "  Building $VARIANT variant (mod=$MOD_ID)"
  echo "=========================================="

  local AD=/tmp/appimg/OpenXcom-CHT-v2.20-$VARIANT.AppDir
  rm -rf $AD
  ...
  # 2. .so deps (skip glibc/GL/X11/wayland)
  ...
  for lib in $LIBS; do
    ...
    case "$base" in
      ...
      libnsl*|ld-linux*) ;;
      *) cp -L "$lib" $AD/usr/lib/ 2>/dev/null && BUNDLED=$((BUNDLED+1)) ;;
    esac
  done
  echo "  bundled $BUNDLED .so libs, $(du -sh $AD/usr/lib | cut -f1)"

  # 3. Game data — common + standard 共用，only variant-specific game data
  cp -r $BIN/common   $AD/usr/share/openxcom/
  cp -r $BIN/standard $AD/usr/share/openxcom/
  cp -r $BIN/$DATA_DIR $AD/usr/share/openxcom/

  # 4. AppRun (variant-specific mod id)
...
Name[zh_TW]=OpenXcom 繁中 $VARIANT v2.20
Comment=$VARIANT (X-COM zh-TW localized)
Exec=openxcom
...
DESKTOP_EOF
  ...
  # 6. Icon (reuse signature_pang chibi face)
  python3 << PY_EOF
...
PY_EOF
  ...
  echo "  AppDir size: $(du -sh $AD | cut -f1)"

  # 7. Build AppImage
  local OUT_NAME="OpenXcom-CHT-v2.20-$VARIANT-x86_64.AppImage"
  ARCH=x86_64 $APPIMG_TOOL --appimage-extract-and-run $AD $OUT/$OUT_NAME 2>&1 | tail -5

  if [ -f "$OUT/$OUT_NAME" ]; then
    chmod +x $OUT/$OUT_NAME
    echo "  -> $OUT/$OUT_NAME ($(du -sh $OUT/$OUT_NAME | cut -f1))"
  else
    echo "  FAILED: $OUT_NAME not produced"
    return 1
  fi
}

# Need to recreate appimagetool dir if cleared
if [ ! -f $APPIMG_TOOL ]; then
  mkdir -p /tmp/appimg
  wget -q https://github.com/AppImage/AppImageKit/releases/download/continuous/appimagetool-x86_64.AppImage -O $APPIMG_TOOL
  chmod +x $APPIMG_TOOL
...
ls -lh $OUT/OpenXcom-CHT-v2.20-*.AppImage 2>/dev/null
