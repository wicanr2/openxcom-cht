# v2.29 quad-pack — same as v2.22 logic, new filename
$ErrorActionPreference = "Stop"
$Build = "D:\openxcom\OpenXcom\build-win64-release\bin\Release"
$Bin   = "D:\openxcom\OpenXcom\bin"
$Out   = "D:\openxcom\dist"
$Tmp   = "$Out\_v2_29_staging"
if (Test-Path $Tmp) { Remove-Item $Tmp -Recurse -Force }

function Build-Variant {
    param([string]$Variant, [string]$ModId, [string]$GameDataDir)
    Write-Host ("==> v2.29 " + $Variant + " (mod=" + $ModId + ")") -ForegroundColor Cyan
    $Pkg = "$Tmp\OpenXcom-CHT-v2.29-$Variant-portable"
    ...
    $ZipPath = "$Out\OpenXcom-CHT-v2.29-$Variant-portable.zip"
...
Write-Host ""
Get-ChildItem "$Out\OpenXcom-CHT-v2.29-*" | Select-Object Name, @{N='MB';E={[math]::Round($_.Length/1MB,2)}} | Format-Table -AutoSize
